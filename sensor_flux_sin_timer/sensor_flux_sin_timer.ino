...
unsigned long relayTimeOnMillis= 10000; // en milisegundos 60000
unsigned long flowSensorOfTimeMillis= 20000; // en milisenduos 60000
unsigned long measureTimeWindow = 60000; // en milisegundos (60000) //tiempo tomado en total para calcular la flowRate
...
void setup() {
  ...
  //Timer1.initialize(relayTimeOnMillis);  // Configura TimerOne para el tiempo de encendido del relé
...
void loop() {
  ...
  if (!ignoreButton && buttonState == HIGH) {
    Serial.println("Button Pressed");    // turn LED on:
    startRelay();
    delay(1000);
  }
...
void waitMillis(long waitMillis){
  ...
  Serial.println("WaitMillis starts at " + String(initMillis)); 
  while(millis() - initMillis < waitMillis){
    delay(100); //miliseconds
    Serial.print("/");
  }
  Serial.println("\nWaitMillis ends at " + String(millis() - initMillis)); 
}
...
void buttonTimerInterrupt() {
  Serial.println("buttonTimerInterrupt");    // turn LED on:
  stopRelay();
  // Inicializar tiempo de encendido despues de presionar el boton
  waitMillis(flowSensorOfTimeMillis);
  activateFlowSensor();
}
